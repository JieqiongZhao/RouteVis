			...
			$.getJSON("output"+j+".txt", function(result) {
					...
					result.resourceSets[0].resources.length > 0)
				{
					...
					for(var i = 0; i < pathIndices.length; i++){
						routepoints[i] = new Location(routeline.coordinates[pathIndices[i]][0], routeline.coordinates[pathIndices[i]][1]);
						//arrowarray[i]=new Location(routeline.coordinates[pathIndices[i]][0], routeline.coordinates[pathIndices[i]][1]);
					}
					*/
					...
					for(var i = 1; i < pathIndices.length; i++){
						startx = lineData[pathIndices[i] - 10].x;
						starty = lineData[pathIndices[i] - 10].y;
						//startx = lineData[pathIndices[i-1]].x;
						//starty = lineData[pathIndices[i-1]].y;
						endx = lineData[pathIndices[i]].x;
						endy = lineData[pathIndices[i]].y;
						//arrowarray[i - 1] = new Arrow(startx, starty, endx, endy);
						
						if(endx > startx && endy < starty){
							alpha = Math.atan((endx - startx)/(starty - endy));
							x = endx - 10*Math.sin(alpha - 30/180*Math.PI);
							y = endy + 10*Math.cos(alpha - 30/180*Math.PI);
							arrowData.push({"x": x, "y": y});
							arrowData.push({"x": endx, "y": endy});
							alpha = Math.atan((starty - endy)/(endx - startx));
							x = endx - 10*Math.cos(alpha - 30/180*Math.PI);
							y = endy + 10*Math.sin(alpha - 30/180*Math.PI);
							arrowData.push({"x": x, "y": y});
