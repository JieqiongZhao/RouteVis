...
function Load (){
	...
	var overlapsegs = [];
	$.ajax({
		url: 'result.txt', async: false, dataType: 'json', success: function(sampl){
			overlapsegs = sampl;
		}
	});
	...
	var shiftline = [];
	var directions = [];
	colortable = ["rgba(102, 194, 165,","rgba(252, 0, 0,","rgba(0, 0, 255,","rgba(231, 138, 195,","rgba(166, 216, 84,","rgba(102, 194, 165,"];
	colortable2 = ["rgba(102, 194, 165, 0.25)","rgba(252, 0, 0, 0.25)", "rgba(0, 0, 255, 0.25)","rgba(231, 138, 195, 0.25)","rgba(166, 216, 84, 0.25)","rgba(102, 194, 165, 0.25)"];
	for(var k = 0; k < total.length; k++)
	{
		var tmpindex = [];
		tmpindex.push(total[k].resourceSets[0].resources[0].routePath.generalizations[0].pathIndices);
		pathlength = total[k].resourceSets[0].resources[0].routePath.line.coordinates.length;
		colorincrease = 1.0/pathlength;
		pathsublength = 0;
		...
		// var gradient = d3.select("body").append("svg").append("defs")
  // 		.append("linearGradient")
	 ...
	 //    .attr("spreadMethod", "pad");
		...
		// .attr("offset", "0%")
		// .attr("stop-color", colortable[k])
		// .attr("stop-opacity", 1);

		// gradient.append("svg:stop")
	 //    .attr("offset", "100%")
	 //    .attr("stop-color", colortable2[k])
	 //    .attr("stop-opacity", 1);
	 	var currtmp = [];
	 	for(var cc = 1; cc < shiftline.length; cc++)
	 	{
	 		currtmp.push(shiftline[cc-1]);
	 		currtmp.push(shiftline[cc]);
	 		//currtmp.push(shiftline[cc+1]);

	 		var lineGraph = svgContainer.append("path")
			.attr("d", lineFunction(currtmp))
			.attr("stroke", colortable[k]+(1.0 - (cc-1)*colorincrease)+")")
			.attr("stroke-width", 5)
			.attr("fill", "none");
