...
function Load (){
	...
	for(var k = 0; k < total.length; k++)
	{
		...
		for(var l = 1; l < tmpindex[0].length; l++)
		{
			...
			//check unit vector direction
			if(pathinfo.length > 0)
			{
				prevx = pathinfo[pathinfo.length-1].unitx;
				prevy = pathinfo[pathinfo.length-1].unity;

				cosvalue = (prevx * unitx + prevy * unity);
				if(cosvalue < 0)
				{
					unitx = -unitx;
					unity = -unity;
			...
			uni1x = (lo1 + lo2)/2;
			uni1y = (la1 + la2)/2;
			y1tmp = (uni1y - lat1)/delY + 50;
			x1tmp = (uni1x - long1)/delX + 50;
			unitall.push({"x": x1tmp, "y": y1tmp});
			uni1x = (unitx*0.2 + (lo1 + lo2)/2);
			uni1y = (unity*0.2 + (la1 + la2)/2);
			y1tmp = (uni1y - lat1)/delY + 50;
			x1tmp = (uni1x - long1)/delX + 50;
			unitall.push({"x": x1tmp, "y":y1tmp});
			pathinfo.push({"id":k, "sinex":tmpindex[0][l-1], "eindex": tmpindex[0][l], "unitx": unitx, "unity": unity, "ofx": offsetx, "ofy": offsety});
			
			for(var h = tmpindex[0][l-1] ; h < tmpindex[0][l] - 2; h++ ) //every point in this line segment
			{
				...
				maxoffset = -1;
				for(var q = 0; q < overlapsegs.length; q++)
				{

					// if((overlapsegs[q][0] == k) && (overlapsegs[q][2] != overlapsegs[q][0]) ) // if current path has a overlapping
					// {
							...
							for(var p = 0; p < pathinfo.length; p++)
							{
								if(pathinfo[p].id == overlapsegs[q][0])
								{
									...
									if((pathinfo[p].sinex <= coindex) && (pathinfo[p].eindex >= coindex))
									{
										...
										else if((pathinfo[p].ofx == 5)/* && (pathinfo[p].ofx >= maxoffset)*/)
										{
											offsetx = 6;
											offsety = 6;
											maxoffset = 6;
				...
				lo4 = newlo + offsetxtmp;
				la4 = newla + offsetytmp;
				y1 = (la4 - lat1)/delY + 50;
			...
			// var Lineunite = svgContainer.append("path")
			// .attr("d", lineFunction2(unitall))
			// .attr("stroke", "#000000")
			// .attr("stroke-width", 2)
			// .attr("fill", "none");
