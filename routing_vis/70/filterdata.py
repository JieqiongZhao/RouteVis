...
#         print start, end
        ...
        for l in range(0,len(listOfDot1) - 1):
            for k in range(0,len(listOfDot2) - 1):
                if line_intersection(listOfDot1[l][2],listOfDot1[l][3],listOfDot1[l + 1][2],listOfDot1[l + 1][3],listOfDot2[k][2],listOfDot2[k][3],listOfDot2[k + 1][2],listOfDot2[k + 1][3]):
                    #print(str(route1)+","+str(listOfDot1[l][1])+","+str(route2)+","+str(listOfDot2[k][1]))
                    couter += 1;
                    if couter <= 3:
                        pointoutdata.append([route1,listOfDot1[l][1],route2,listOfDot2[k][1]])


# with open('point.txt', 'w') as outfile:
#     json.dump(pointoutdata, outfile)
print("end");
print(strftime("%Y-%m-%d %H:%M:%S", gmtime()))
